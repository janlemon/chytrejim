  ...
  return (
    <SafeAreaView style={{ flex: 1, padding: theme.space.xl, backgroundColor: tokens.bg }}>
      <View style={{ flex: 1, gap: 16, paddingHorizontal: 20 }}>
        ...
        <TouchableOpacity onPress={() => setConsentTerms(!data.consent_terms)} style={{ flexDirection: 'row', gap: 12, alignItems: 'flex-start' }}>
        ...
        <TouchableOpacity onPress={() => setConsentPrivacy(!data.consent_privacy)} style={{ flexDirection: 'row', gap: 12, alignItems: 'flex-start' }}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12, width: '100%', maxWidth: 360 }}>
          ...
          <TouchableOpacity testID="onboarding-next" onPress={() => router.push('/(onboarding)/review')} disabled={!data.consent_terms || !data.consent_privacy} style={[invertedButtonStyle, { flex: 1, opacity: (!data.consent_terms || !data.consent_privacy) ? 0.6 : 1 }]}>
